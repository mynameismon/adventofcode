...
fn parse_line(line: String) -> Engine {
    ...
    Engine {
        engines: groups.next().unwrap().to_string(),
        operational: groups
            .next()
            .unwrap()
            .split(",")
...
fn calculate_combinations(e: Engine) -> u64 {
    ...
    let mut dp: Vec<Vec<Vec<u64>>> =
        vec![vec![vec![0; engines.len() + 1]; operational.len() + 1]; engines.len() + 1];
...
pub fn solve() {
    ...
    if cfg!(debug_assertions) {
        println!("[Answer to part 2 hidden due to slow down]");
    } else {
